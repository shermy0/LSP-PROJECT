...
<div class="container-fluid px-4 py-4">
                ...
                <div class="col-md-6">
                    ...
                    <div>
                        @if($persetujuan->status == 'draf')
                            <span class="badge bg-warning text-dark px-3 py-2 fs-6">Draf (Menunggu Tanda Tangan Anda)</span>
    ...
    <!-- Card TUK (dengan jenis dan alamat) -->
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-header bg-white border-0 pt-4 pb-0">
            <div class="d-flex align-items-center">
                <div class="bg-primary bg-opacity-10 p-3 rounded-3 me-3">
    ...
    <!-- Card Pernyataan (sesuai dokumen) -->
    ...
    <!-- Card Tanda Tangan + Setuju Asesmen (digabung) -->
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-header bg-white border-0 pt-4 pb-0">
            <div class="d-flex align-items-center">
                <div class="bg-primary bg-opacity-10 p-3 rounded-3 me-3">
                ...
                <div>
                    <h5 class="fw-bold mb-0">Konfirmasi dan Tanda Tangan</h5>
                    <p class="text-secondary mb-0 small">Setujui asesmen dan beri tanda tangan</p>
                </div>
            </div>
        </div>
        <div class="card-body pt-3">
            ...
            @else
                @if(Auth::user()->role == 'asesi' && $persetujuan->status == 'draf')
                    {{-- Form tanda tangan asesi dengan checkbox setuju --}}
                    <form method="POST" action="{{ route('asesi.persetujuan_asesmen.signature', $persetujuan->id_persetujuan) }}" id="signatureForm">
                        @csrf

                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-check mb-3">
                                    <input class="form-check-input" type="checkbox" name="setuju_asesmen" value="1" id="setujuCheckbox" required>
                        ...
                        (function() {
                            ...
                            document.getElementById('signatureForm').addEventListener('submit', function(e) {
                                if (!setujuCheckbox.checked) {
                                    e.preventDefault();
                                    alert('Anda harus menyetujui asesmen terlebih dahulu.');
                                    return;
                                }
                                if (isCanvasBlank()) {
                                    e.preventDefault();
                                ...
                                document.getElementById('ttd-asesi-input').value = canvas.toDataURL('image/png');
                            });
                        })();
                    </script>
                @else
                    <p class="text-muted">Tidak ada tindakan yang dapat dilakukan saat ini.</p>
                @endif
            @endif
        </div>
    </div>

    <!-- Tombol Kembali ke Daftar Persetujuan Asesi -->
