...
<form action="{{ route('form_perencanaan.fr_va_asesor', ['periode' => $periode, 'skema_id' => $skema_id ?? '']) }}" method="GET">
    @csrf
    ...
    @foreach($orangRelevan as $id => $label)
        <div class="mb-3">
            ...
            <div id="{{ $id }}Form" class="mt-2" style="display:none;">
                ...
                <button type="button" class="btn btn-success btn-sm addNama" data-target="{{ $id }}-namaList">+ Tambah</button>
            </div>
        </div>
    @endforeach
</div>
...
document.addEventListener("DOMContentLoaded", function () {
    // fungsi template untuk form input baru
        ...
        <div class="border p-2 rounded">
            <div id="${name}-namaList">
                <div class="input-group mb-2">
                    ...
                    <input type="text" name="${name}_diskusi[]" class="form-control" placeholder="Masukkan hasil diskusi">
                    <button class="btn btn-danger removeNama" type="button"><i class="fa fa-trash"></i></button>
                </div>
            </div>
            <button type="button" class="btn btn-success btn-sm mb-3 addNama" data-target="${name}-namaList">+ Tambah</button>
        </div>`;
    }

    // toggle section saat checkbox dicentang
    document.querySelectorAll(".toggle-section").forEach(checkbox=>{
        checkbox.addEventListener("change",function(){
            ...
            if(this.checked){
                ...
                // event delegation untuk tambah/hapus input
                target.addEventListener("click",function(e){
                    if(e.target.classList.contains("addNama")){
                        ...
                        div.innerHTML = `
                            ...
                            <input type="text" name="${checkbox.id}_diskusi[]" class="form-control" placeholder="Masukkan hasil diskusi">
                            <button class="btn btn-danger removeNama" type="button"><i class="fa fa-trash"></i></button>`;
